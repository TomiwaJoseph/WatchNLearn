...
def instructor_profile(request, user_first_name, user_id):
    query = Instructor.objects.filter(user__first_name=user_first_name, id=user_id)
    if not query.exists():
        return redirect("courses")
    instructor_courses = Course.objects.filter(instructor=query.first().user, status='published')
    context = {
...
def profile(request, user_first_name, user_id):
    ...
    if not query.exists():
        return redirect("courses")
    my_courses = Course.objects.filter(instructor=request.user, status='published')
    context = {
...
def create_module(request):
    ...
    if request.method == "POST":
        ...
        module_create_form.fields['course'].queryset = Course.objects.filter(instructor=request.user, status="draft")
...
def create_content(request):
    ...
    if request.method == "POST":
        ...
        content_create_form.fields['module'].queryset = Module.objects.filter(course__instructor=request.user, course__status="draft")
...
def my_learning(request):
    ...
    my_courses = [course for course in all_courses if course.instructor != request.user]
    context = {
        'my_courses': my_courses,
    }
